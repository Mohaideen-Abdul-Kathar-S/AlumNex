...
@app.route("/groups/<community_id>", methods=["GET"])
...
@app.route("/message_versions/<message_id>", methods=["GET"])
...
@app.route("/snapshots/<group_id>", methods=["GET"])
...
@app.route("/snapshot/messages/<snapshot_id>", methods=["GET"])
...
@app.route("/snapshot/<snapshot_id>", methods=["POST"])
...
@app.route("/parse_id_card", methods=["POST"])
def parse_id_card():
    if "file" not in request.files:
    ...
    try:
        ...
        # Extract text from file (reuse logic from async version)
        ...
        # Save to MongoDB
        result = db.idcards.insert_one(structured_data_json)
        structured_data_json["_id"] = str(result.inserted_id)

        return jsonify({"status": "success", "data": structured_data_json})
